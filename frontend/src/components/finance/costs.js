...
export class Costs {
    ...
    async choiceDelCategory(element) {
        if (this.commonParams.currents.currentCostCtg) {
            const result = await HttpUtils.request('/categories/expense/' + this.commonParams.currents.currentCostCtg, 'DELETE');
            await this.openNewRoute('/costs');
        }
    }
